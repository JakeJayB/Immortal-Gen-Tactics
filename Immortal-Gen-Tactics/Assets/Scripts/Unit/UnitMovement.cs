    ...
    public static void MoveUnit(Unit unit, Vector2Int direction)
    {
        ...
        if (destination != null)
        {
           unit.gameObject.transform.position = new Vector3(
               (destination.CellLocation.x) * TileProperties.TILE_WIDTH,
               (destination.CellLocation.y) * TileProperties.TILE_HEIGHT + TileProperties.TILE_HEIGHT, 
               (destination.CellLocation.z) * TileProperties.TILE_LENGTH);

           unit.CellLocation = destination.CellLocation;
        }
    }
}
